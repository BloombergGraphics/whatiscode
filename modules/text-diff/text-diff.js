  ...
  function diffToWords(diff){
    ...
    d3.select('#width-calc').append('div')
      ...
      .dataAppend(rv, 'div').append('div')
        .text(ƒ('value'))
        .each(function(d){
          if (widths[d.value]) return d.width = widths[d.value]

          console.log(d)
          d.width = this.getBoundingClientRect().width
          d.width = (d.value == ' ' ? 4.5 : d.width) + 'px'
          widths[d.value] = d.width
        })
  ...
  function updateFalling(curDiffIndex){
    ...
    if (module.active){
      ...
      d3.select('[data-module="text-diff"]').dataAppend(wordDiffs, 'div.falling')
          .html(function(d){ return d.value == ' ' ? '<wbr>&nbsp;</wbr> ' : d.value })
          .style('opacity', function(d){ return d.value == ' ' ? '0' : '1' })
