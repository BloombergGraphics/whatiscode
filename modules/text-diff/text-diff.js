...
function diffToWords(diff){
  ...
  diff.forEach(function(obj, i){
    obj.value.split(' ').forEach(function(chr, j, array){
      var lastChar = ' '
      var nextChar = diff[i + 1] && diff[i + 1].value[0]
...
function updateFallingV(curDiffIndex){
  var wordDiffs = diffToWords(diffs[curDiffIndex])

  d3.selectAll('#text-diff').html('')

  d3.select('#text-diff').dataAppend(wordDiffs, 'span.falling')
      .text(ƒ('value'))
  ...
  // debugger
  window.setTimeout(function(){ updateFallingV(++curDiffIndex % diffs.length) }, 5500)
}
